...
namespace Assets.Scripts.Entities.Player
{
    public class DoDamage : MonoBehaviour
    {
        ...
        private void CheckDamage()
        {
            if (Animator == null)
                return;
            var toRemove = new List<Collision>(); //Create list of colliders that needs to be removed from the list colliders 

            foreach(var c in _collisions) //For each collider check if it has an entity script and remove if it does
            {
                if(c.gameObject.GetComponent<Entities.Entity>() != null && Animator.GetCurrentAnimatorStateInfo(0).IsTag("attack")) // Also check if animation is playing
                {
                    Instantiate(DamageEffect, c.contacts[0].point, Quaternion.FromToRotation(Vector3.up, c.contacts[0].normal));
                    var healthScript = c.gameObject.GetComponent<Entities.Entity>(); // Call entity script of the hit entity
